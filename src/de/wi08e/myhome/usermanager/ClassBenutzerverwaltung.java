...
public class ClassBenutzerverwaltung {
	...
	{
		//Rolle des Users in die jeweiligen Rechte �bersetzen.
	...
	{	//Rolle definieren und zugeh�rige Rechte hinzuf�gen. bei misserfolg, wird false zur�ck gegeben.
		Right bufferRight = new Right();
		try {
			connect_db();
	...
	{ //Funktion gibt false zur�ck falls Eintrag bereits existiert. True wenn geklappt.
		try {
			connect_db();
			int writeInt = 0;
			if (writable == true)
	...
	{	//Funktion l�scht das �bergebene Recht in der Rolle. Gibt bei Erfolg true zur�ck bei nicht existenz oder misserfolg false.
		try {
			connect_db();
